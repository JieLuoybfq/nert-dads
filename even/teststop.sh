...
{
 $|=1;
 my(@hosts)=("4.2.2.2","8.8.8.8","8.8.4.4","139.130.4.5","4.2.2.1","4.2.2.3","4.2.2.4","64.68.200.200","24.26.192.184","4.69.145.136");
 my($result)=0;
 my($host)=0;
 my($rndh)=0;
 my($connection)=0;
 ...
 while (1)
 {
   ...
   if ($connection==0) {
	...
	if ($sumt<1) {
	 ...
	 if ($attempts>1) {
	   $attempts=0;
	   print " ==> Network is down, " . (10-$sumt) . " servers seem to have lost connection.\nRestarting netwok services...\n";
	   system("sudo stop network-manager");
	   system("sudo start network-manager"); 
